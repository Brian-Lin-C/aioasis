...
console.log(await evalJs(`(()=>{const a=[...document.querySelectorAll('header nav a')].find(a=>/观测/.test(a.textContent));const r=a.getBoundingClientRect();const cx=r.x+r.width/2,cy=r.y+r.height/2;const el=document.elementFromPoint(cx,cy);return JSON.stringify({cx,cy,href:a.getAttribute('href'),hit:el?el.tagName+'.'+el.className.toString().slice(0,60):'null',inside:a.contains(el)})})()`))
process.exit(0)
