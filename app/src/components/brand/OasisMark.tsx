...
export default function OasisMark({ className = '' }: { className?: string }) {
  return (
    <svg viewBox="0 0 100 100" className={className} aria-hidden focusable="false">
      ...
      {/* 微缩棕榈：源 1024 viewBox 缩至高约 38，岛基落于水面线（y=55）不重叠，昼间剪影色 */}
      <path
        d={PALM_D}
        transform={`translate(50 55) scale(${38 / (PALM_CONTENT_BOTTOM - 10)}) translate(${-PALM_CONTENT_CX} ${-PALM_CONTENT_BOTTOM})`}
        fill="#5c5236"
      />
    </svg>
  )
