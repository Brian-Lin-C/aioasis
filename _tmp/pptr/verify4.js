...
(async () => {
  const browser = await puppeteer.launch({
    ...
    headless: false,
    args: ['--window-size=2033,760', '--window-position=0,0'],
  });
  ...
  await page.screenshot({ path: 'headful-mid1.png' });
  await new Promise(r => setTimeout(r, 200));
  await page.screenshot({ path: 'headful-mid2.png' });
  await new Promise(r => setTimeout(r, 1500));
  const clips = await page.evaluate(() => window.__clips);
  console.log('CLIPS:', JSON.stringify(clips));
  await page.screenshot({ path: 'headful-after.png' });
  await browser.close();
  console.log('done');
